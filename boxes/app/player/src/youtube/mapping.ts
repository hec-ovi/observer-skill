...
/** `onError` codes. 153 is the one a 2026 page actually hits, and it needs its own words. */
const ERRORS = new Map<number, { code: PlayerErrorCode; hint?: string }>([
  [2, { code: 'VIDEO_UNAVAILABLE', hint: 'YouTube does not recognise this video id. Paste the link again.' }],
  [
    5,
    {
      code: 'PLAYER_UNAVAILABLE',
      hint: "YouTube's player refused this video. Reload the page, and pick another video if it happens again.",
    },
  ],
  ...
  [
    ...
    {
      code: 'NOT_EMBEDDABLE',
      hint: 'YouTube rejects embeds that arrive without a referrer. Open this page over http or https and leave the referrer policy at strict-origin-when-cross-origin; file:// pages, extension pages, and custom app schemes never send one.',
    },
  ],
])
