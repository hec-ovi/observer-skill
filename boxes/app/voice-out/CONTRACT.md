...
- `warm` downloads and compiles whatever the provider needs so the first spoken line is not
  the slow one. Safe to call repeatedly.
...
`INVALID_VOICE_CONFIG`, `VOICE_UNAVAILABLE`. Nothing else throws. A provider that cannot
run falls back to `web-speech` when the browser has it, and only reports
`VOICE_UNAVAILABLE` when there is no way to make a sound.
...
Every provider is one file exporting `{ id, available(), warm(), speak(text, opts) }` and a
registry line. The tests drive `speak` with each provider faked at its own boundary (fake
worker, fake `speechSynthesis`, fake fetch) and assert the event sequence and the cut-off
behaviour. The Pocket provider's model URLs and worker protocol are documented in
`docs/pocket.md` inside this box.
